...
if [[ $CHECK -eq 1 ]]; then
  ...
  elif id styr >/dev/null 2>&1 && run_as_styr bash -lc 'command -v claude' >/dev/null 2>&1; then
...
if [[ $NO_CLAUDE -eq 0 ]]; then
  if run_as_styr bash -lc 'command -v claude' >/dev/null 2>&1; then
  ...
  claude_real="$(run_as_styr bash -lc 'command -v claude' 2>/dev/null || true)"
