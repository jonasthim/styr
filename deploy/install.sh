...
before="$(normalize_version "$(installed_version)")"
if [[ -n "$BINARY" ]]; then
...
MSG
after="$(normalize_version "$(installed_version)")"
if [[ "$before" == "$after" ]]; then
  echo "styr install: OK, already current at $after"
elif [[ "$before" == "(not installed)" || "$before" == "unknown" ]]; then
  echo "styr install: OK, installed $after"
else
  echo "styr install: OK, upgraded $before -> $after"
fi
