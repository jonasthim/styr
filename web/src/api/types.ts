...
export type Run = Omit<Schemas['Run'], 'report'> & {
  ...
  iteration: number
  /** Set on every run a pipeline step started (T54); null otherwise. */
...
export interface Schedule {
  ...
  name: string
  /** '' when this schedule runs a pipeline instead (T54). */
  template_id: string
  /** The pipeline this schedule starts, or null when it runs a template. */
  pipeline_id: string | null
  cron: string
  enabled: boolean
...
export interface ScheduleInput {
  name: string
  /** Exactly one of `template_id` / `pipeline_id` (T54). */
  template_id?: string
  pipeline_id?: string | null
  cron: string
  vars?: Record<string, unknown>
